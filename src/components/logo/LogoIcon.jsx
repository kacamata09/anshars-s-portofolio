...
export default function LogoIcon() {
  ...
  return (
    <Box
      className="icon-logo"
      sx={{
        width: { xs: 40, sm: 50, md: 60 },  // Menyesuaikan ukuran logo (lebih besar dari sebelumnya)
        height: { xs: 40, sm: 50, md: 60 },  // Menyesuaikan tinggi logo (lebih besar dari sebelumnya)
        position: 'relative',
        cursor: 'pointer',
        display: 'block',
    ...
    >
      {logoIconPath ? (
        <CardMedia src={logoIconPath} component="img" alt="logo" sx={{ height: '100%' }} loading="lazy" />
      ) : (
